...
#         self.start_layer= AttentionStem(in_channels=3, out_channels=first_num_filters, kernel_size=4, stride=1, padding=2, groups=1)

        self.start_layer = nn.Conv2d(in_channels = 6, out_channels = first_num_filters, kernel_size =3, stride=1, padding=1,bias=True)
...
class standard_block(nn.Module):
    ...
    def __init__(self, filters, projection_shortcut, strides, first_num_filters) -> None:
        ...
        self.conv1 = nn.Conv2d(filters//strides, filters, kernel_size=3, stride=strides, padding=1, bias=True)
        self.bn_relu1 = batch_norm_relu_layer(filters,eps=1e-5, momentum=0.1)
        self.conv2 = nn.Conv2d(filters, filters, kernel_size=3, stride=1, padding=1, bias=True)
...
class bottleneck_block(nn.Module):
    def __init__(self, filters, projection_shortcut, strides, first_num_filters) -> None:
        ...
        self.conv1 = nn.Conv2d(in_filters, hidden_filters, kernel_size=1, stride=self.strides, padding=0,bias=True)
        self.bn_relu2 = batch_norm_relu_layer(hidden_filters,eps=1e-5, momentum=0.1)
        self.conv2 = AttentionConv(hidden_filters, hidden_filters, kernel_size=7, stride=1, padding=3, groups=1,bias=True)
        # conv2 is attention conv
        self.bn_relu3 = batch_norm_relu_layer(hidden_filters,eps=1e-5, momentum=0.1)
        self.conv3 = nn.Conv2d(hidden_filters, self.filters, kernel_size=1, stride=1, padding=0,bias=True)
            ...
            if block_fn is standard_block and i== 0:
                 if first_num_filters != filters_out:
                    projection_shortcut = nn.Sequential(
                                              nn.Conv2d(filters_out//2, filters_out, stride=strides, kernel_size=1,bias=True),
                ...
                if first_num_filters == filters_out // 4 and i == 0:
                    projection_shortcut = nn.Sequential(
                            nn.Conv2d(filters_out//4, filters_out, kernel_size = 1, stride=strides, padding=0,bias=True),
                            nn.BatchNorm2d(filters_out,eps=1e-5, momentum=0.1)
                    )
                elif first_num_filters < filters_out and i == 0:
                    projection_shortcut = nn.Sequential(
                            nn.Conv2d(filters_out//2, filters_out, kernel_size = 1, stride=strides, padding=0,bias=True),
                            nn.BatchNorm2d(filters_out,eps=1e-5, momentum=0.1)
                    )
